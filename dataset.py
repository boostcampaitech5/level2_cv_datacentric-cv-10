...
class SceneTextDataset(Dataset):
    def __init__(
        ...
        # ! Augmentation 추가
        self.color_jitter, self.sharpen = color_jitter, sharpen
    ...
    def __getitem__(self, idx):
        ...
        augment_funcs = []
        if self.color_jitter:  # ! ColorJitter 수행 with p=0.5
            augment_funcs.append(
                A.ColorJitter(
                    brightness=0.5, contrast=0.5, saturation=0.5, hue=0.25, p=0.5
                )
            )
        if self.sharpen:  # ! Sharpen 수행 with p=0.5
            augment_funcs.append(
                A.Sharpen(alpha=(0.2, 0.5), lightness=(0.5, 1.0), p=0.5)
            )
        if self.random_brightness_contrast:  # ! RandomBrightnessContrast 수행 with p=0.5
            augment_funcs.append(
                A.RandomBrightnessContrast(
                    brightness_limit=(-0.22, 0.22),
                    contrast_limit=(-0.22, 0.22),
                    brightness_by_max=True,
                    p=0.3,
                )
            )
        if self.normalize:  # ! Normalize 수행 with p=1.0
            augment_funcs.append(A.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)))
        album_transform = A.Compose(augment_funcs)
